...
function _render(node, object) {
    ...
    if ("text" === node.type) {
        ...
        el = document.createTextNode(process(content));
        return el;
    }
    if ("element" === node.type) {
        el = document.createElement(node.tagName);
        for (let attr of node.attributes) {
            if ("v-if" === attr.name && !evalInScope(attr.value, object)) {
                return null;
            }
            // todo 处理 {{}} 转换
            if (attr.value.match(/^{{.+}}$/)) {
        ...
        for (let child of node.children) {
            let childEl = _render(child, objectGetter);
            if (childEl !== null) {
...
function process(str, object) {
    ...
    while (true) {
        ...
        current = index + tempResult.value[0].length;
    }
